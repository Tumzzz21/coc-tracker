(function () {
  ...
  // Registration is closed unless the server turns it on, so the block starts hidden.
  fetch('/api/config')
    .then((response) => response.json())
    .then((payload) => {
      if (payload && payload.data && payload.data.allowRegistration) {
        const block = document.querySelector('#register-block');

...
const { ensureColumn } = require('../config/schema');
...
async function initializeTables() {
  ...
  for (const [column, definition] of attendanceColumns) {
    ...
    await ensureColumn('capital_attendance', column, definition);
  }
  for (const [column, definition] of sessionColumns) {
    await ensureColumn('war_sessions', column, definition);
    await ensureColumn('capital_sessions', column, definition);
  }
}
...
router.put('/:type/:id/attendance', requireAdmin, async (req, res, next) => {
  const selected = getConfig(req.params.type);
  const id = positiveId(req.params.id);
  if (!selected) return res.status(404).json({ error: 'Unknown session type.' });
  if (!id) return res.status(400).json({ error: 'Session id must be positive.' });
...
router.post('/:type/:id/finish', requireAdmin, async (req, res, next) => {
  const selected = getConfig(req.params.type);
  const id = positiveId(req.params.id);
  if (!selected) return res.status(404).json({ error: 'Unknown session type.' });
  if (!id) return res.status(400).json({ error: 'Session id must be positive.' });

...
const { requireAdmin, initializeUserTable, allowRegistration } = authRoutes;
...
// Public flags the login page needs before anyone has a token.
app.get('/api/config', (req, res) => {
  res.json({ data: { allowRegistration } });
});
...
app.put('/api/settings', requireAdmin, async (req, res, next) => {
  ...
  // Quotes, spaces, and parentheses are refused because the value is injected into a
  // CSS url() on the client, where they could break out and inject other styles.
  if (value && !/^https?:\/\/[^\s"'()<>\\]+$/i.test(value)) {
    return res.status(400).json({
      error: 'bgImageUrl must be an http or https URL without quotes, spaces, or parentheses.'
    });
  }
...
async function startServer() {
  try {
    ...
    // A provisioning problem must not stop the app; /health reports the database state.
    console.error('Unable to prepare the user table before startup.', error);
  }
  try {
    await sessionRoutes.initializeSessionTables();
  } catch (error) {
    // Serving anyway keeps the UI available while session storage is repaired.
